...
int main(int argc, char **argv) {
    ...
    unsigned compression_level, ncores;
    ...
    try {
		std::tie(interval, start, stop, hdf5_path, rts_path, instruments_path, trading_days_path, compression_level, ncores, convert) = std::make_tuple(
		...
		if(files.size()) {
			...
			<< " file" << (files.size() > 1 ? "s" : "") 
			<< " into HDF5: using 1 thread — © IEX + Varga Consulting, 2017–2025\n"
			...
			for(std::string path: files)
				all_tasks.emplace_back(
					pool.submit_task( io::task<consumer>(path, start, interval, stop, fd, dcpl, rts, is_irts_enabled, is_rts_enabled)));
