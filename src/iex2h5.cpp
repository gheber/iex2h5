...
int main(int argc, char **argv) {
	...
	std::string output,stream,rts,
			instruments,days,day_begin,day_end,cmd;
    ...
    try {
		std::tie(time_interval, day_begin, day_end, output, rts, instruments, days, gzip, cmd) = std::make_tuple(
		...
		dispatch[cmd]("", output, days, instruments, day_begin, day_end, time_interval );
	} catch( const std::exception& err ) {
		cout << err.what() << endl;
        cout << program << endl;
		return 1;
