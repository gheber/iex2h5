...
TEST_CASE("base64 round-trip string <-> symbol corner cases") {
...
TEST_CASE("base64 round-trip actual symbols") {
    using namespace utils::base64;
    uint16_t id = 0;
    ...
    for(const auto&[iex, symbol]: mock::some::symbols){
        uint64_t base64_encoded_symbol = utils::base64::encode(iex, 0);
        auto [s,i] = utils::base64::decode(base64_encoded_symbol);
        if( auto it = std::ranges::lower_bound(flat_map, base64_encoded_symbol); it != flat_map.end()) {
            auto [x,y] = utils::base64::decode(*it);
            if((base64_encoded_symbol & SYMBOL_MASK) == (*it & SYMBOL_MASK)){
                TRACE << "found:" << i << " <" << s << "> |" << y << "| [" << x << "]" << std::endl;
            } else {
                flat_map.insert(it, base64_encoded_symbol | flat_map.size() );
                TRACE << "insert:" << i << " <" << s << "> |" << y << "| [" << x << "]" << std::endl;
            } 
        } else {
            TRACE << "insert:" << i << " <" << s << "> |" << symbol << "| [" << utils::iex_symbol(iex) << "]" << std::endl;
            flat_map.push_back(base64_encoded_symbol | flat_map.size());
        }
    ...
    for(auto iex: flat_map){
        auto[s,i] = utils::base64::decode(iex);
        TRACE << i << " " << s << std::endl;
    }
}

TEST_CASE("base64 encoding breaks lexicographic sort order") {
    using namespace utils::base64;
...
TEST_CASE("base64-encoded symbol sorting leads to binary search mismatch") {
    using namespace utils::base64;
    ...
    for (const auto& symbol : alphabetical)
        original.push_back(utils::base64::encode(symbol, id++));
    ...
    TRACE << "\nSearching for 'OCEAN   ' in sorted base64-encoded vector:\n";
